...
async function main(): Promise<void> {
  ...
  const parsed = parseChatGptExport(args.input, args.namespace, args.account);
  ...
  printSummary("ChatGPT", { inserted, deduped, failed, items: parsed.items.length });
}

main().catch((error) => {
  process.stderr.write(`ChatGPT import failed: ${String((error as Error)?.message ?? error)}\n`);
  process.exit(1);
});
